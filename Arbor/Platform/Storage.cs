...
public abstract class Storage
{
    ...
    public virtual Storage GetStorageForDirectory(string path)
    {
        if (string.IsNullOrEmpty(path))
            throw new ArgumentException("Must be a non-empty string.", nameof(path));
    ...
    {
        var temporaryPath = Path.Combine(Path.GetDirectoryName(path)!, $"_{Path.GetFileName(path)}_{Guid.NewGuid()}");
